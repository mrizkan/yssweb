...
class Garments extends MY_Controller
{
    var $page = "garment";
    var $img_width = 800;
    var $img_height = 800;
    ...
    {
        parent::__construct();
        $this->load->model('Garments_model', 'model');
...
//        $this->form_validation->set_rules("form[Color]", "Color", "required");
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Item Successfully ' . (!$_id ? "Created." : "Updated.") . '</strong>']);
