...
class Show extends MY_Controller
{
    ...
    {
        parent::__construct();
        $this->load->model('Show_model', 'model');
    ...
    function _form(&$d, $_id = 0)
    {
        ...
        $this->form_validation->set_rules("form[ShowTitle]", "Show Name", "required");
        $this->form_validation->set_rules("form[Image]", "Show Name", "required");
        ...
        if ($this->form_validation->run()) {
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $this->session->set_flashdata('notification', ["alert" => "success", "text" => '<strong> Show Successfully ' . (!$_id ? "Created." : "Updated.") . '</strong>']);
